...
def backup_file(path):
    """
    Create a backup file to JSON. Avoid loosing data if the disk is full.
    """
...
class AttackPrompt(object):
    ...
    def test(self, model, gen_config=None):
        ...
        print(gen_str)
        jailbroken = not any([prefix in gen_str for prefix in self.test_prefixes])  # TODO: redundant wth the following line (em)
...
class MultiPromptAttack(object):
    ...
    def __init__(self, 
        ...
        *args, **kwargs
        ...
        # if there is a list of possible conv templates (different system prompts), then we will cycle through, replacing self.prompts
        self.list_prompts = []  # list of list of PM.
        if workers[0].list_conv_template:
            print('Multiple system prompts passed')
            for i_conv_template in range(len(workers[0].list_conv_template)):
                self.list_prompts.append(
                    [
                        managers['PM'](
                            ...
                            targets,
                            worker.tokenizer,
                            worker.list_conv_template[i_conv_template],
                            control_init,
                            test_prefixes,
                            managers
                        )
    ...
    def get_filtered_cands(self, worker_index, control_cand, filter_cand=True, curr_control=None, filter_token_ids=[]):
        cands, count = [], 0
        worker = self.workers[worker_index]
        for i in range(control_cand.shape[0]):
            decoded_str = worker.tokenizer.decode(control_cand[i], skip_special_tokens=True)
            if filter_cand:
                new_input_ids = worker.tokenizer(decoded_str, add_special_tokens=False).input_ids
                contains_filtered_tokens = any([x in new_input_ids for x in filter_token_ids]) if filter_token_ids else False
                if decoded_str != curr_control and len(new_input_ids) == len(control_cand[i]) and not contains_filtered_tokens:
    ...
    def run(self, 
        ...
        for i in range(n_steps):
            ...
            # if multiple templates, we select one at random
            if self.list_prompts:
                # we stop prev to clean VRAM and avoid outofmemory
                #for worker in self.workers:
                #    worker.stop()
                #    worker.start()
                print(f'sampling one of the {len(self.list_prompts)} system prompts')
                self.prompts = random.choice(self.list_prompts)
            ...
            if loss < best_loss:
                ...
                best_steps = steps
            print(f'[{steps}] Current Loss:', loss, 'Best Loss:', best_loss)

            if self.logfile is not None and (i+1+anneal_from) % test_steps == 0:
                if not return_best_loss:
                    # original behavior: log and report the best loss so far
                    ...
                    # ours: report the actual iteration. Report the best iter later
                    model_tests = self.test_all()
                    self.log(i+1+anneal_from, n_steps+anneal_from, self.control_str, loss, runtime, model_tests, verbose=verbose)

        if return_best_loss:
            self.control_str = best_control
            # log the best iter at the end in 'best'
            model_tests = self.test_all()
            self.log(best_steps+1+anneal_from, n_steps+anneal_from, self.control_str, best_loss, None, model_tests,
                     verbose=verbose, best=True)
            return best_control, best_loss, best_steps
    ...
    def log(self, step_num, n_steps, control, loss, runtime, model_tests, verbose=True, best=False):
        ...
        if not best:
            ...
            log['best'].append({
                ...
                'loss': loss,
                'step': step_num,
                # keep only the executed tests
                'n_passed': [x for x, t in zip(n_passed, total_tests) if t > 0],
                'n_em': [x for x, t in zip(n_em, total_tests) if t > 0],
                'n_loss': [x for x, t in zip(n_loss, total_tests) if t > 0],
            })

        backup_file(self.logfile)
        ...
        if verbose:
            ...
            print((
                f"\n====================================================\n"
                f"{'**BEST** ' if best else ''}Step {step_num:>4}/{n_steps:>4} ({runtime:.4} s)\n"
                f"{output_str}"
                f"control='{control}'\n"
                f"====================================================\n"
            ))
...
class ProgressiveMultiPromptAttack(object):
    ...
    def run(self, 
        ...
        if self.logfile is not None:
            ...
            log['params']['filter_token_ids'] = filter_token_ids
            ...
            log['params']['return_best_loss'] = return_best_loss
        ...
        while step < n_steps:
            ...
            control, loss, inner_steps = attack.run(
                ...
                return_best_loss=return_best_loss,
...
class ModelWorker(object):
    ...
    def __init__(self, model_path, model_kwargs, tokenizer, conv_template, device, list_conv_template=[]):
        ...
        self.model.requires_grad_(False)  # added by us: do not keep grads wrt weights, we just want the grad wrt inputs. Avoid doubling VRAM use :)
        self.tokenizer = tokenizer
        self.conv_template = conv_template
        self.list_conv_template = list_conv_template
...
def get_workers(params, eval=False):
    ...
    for i in range(len(params.tokenizer_paths)):
        ...
        if not tokenizer.pad_token:
            print(f'[INFO] Unknown model. Using default pad token. Check that your model path is correctly supported.')
    ...
    conv_templates = [load_template(template_name, system=None) for template_name in params.conversation_templates]
    print(f"Loaded {len(conv_templates)} conversation templates")
    list_conv_template = []  # for each worker, we have a list of templates
    if params.system_prompts:  # list (models) of lists (string of system prompts)
        for i, template_name in enumerate(params.conversation_templates):
            list_conv_template.append([load_template(template_name, system=system_prompt) for system_prompt in params.system_prompts[i]])
    ...
    workers = [
        ModelWorker(
            ...
            conv_templates[i],
            params.devices[i],
            list_conv_template[i] if list_conv_template else None,
        )
    ...
    print(f'Loaded {num_train_models} train models ({workers[0].model.dtype})')
...
def get_goals_and_targets(params):
    ...
    if params.train_data:
        # we have to load target in str, since it can be composed of only digits '0125' and starts with 0
        train_data = pd.read_csv(params.train_data, dtype={'target': str, 'goal': str})
        ...
        if params.test_data and params.n_test_data > 0:
            test_data = pd.read_csv(params.test_data, dtype={'target': str, 'goal': str})
            ...
            test_targets = train_data['target'].astype(str).tolist()[offset+params.n_train_data:offset+params.n_train_data+params.n_test_data]
            if 'goal' in train_data.columns:
                test_goals = train_data['goal'].astype(str).tolist()[offset+params.n_train_data:offset+params.n_train_data+params.n_test_data]
            else:
                test_goals = [""] * len(test_targets)
